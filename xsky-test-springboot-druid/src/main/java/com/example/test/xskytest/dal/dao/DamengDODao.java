...
import com.example.test.xskytest.dal.mapper.DamengDOMapper;
...
@Repository
@Component
public class DamengDODao {

    @Autowired
    DamengDOMapper damengDOMapper;

    /**
     * 测试用例查询某一张表的信息
     */
    public List<SqlDO> queryTableInformation(String tableName) {
        return damengDOMapper.selectAll(tableName);
    }
    ...
    public void deleteTableInformation(String tableName) {
        damengDOMapper.deleteAll(tableName);
    }
    ...
    public List<SqlDO> mysqlTestException(String tableName, String id) {
        return damengDOMapper.mysqlTestException(tableName, id);
    }
    ...
    public List<SqlDO> slowSql(String tableName, int sleepTime) {
        return damengDOMapper.slowSql(tableName, sleepTime);
    }
