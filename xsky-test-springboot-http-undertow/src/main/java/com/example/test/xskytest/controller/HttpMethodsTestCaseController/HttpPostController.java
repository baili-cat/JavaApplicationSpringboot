...
public class HttpPostController {
    Logger logger = LoggerFactory.getLogger(HttpPostController.class);
     ...
     */
    @ApiOperation(value = "post请求,返回当前时间以及自定义描述信息等list")
     ...
     * @date 2022/11/2 17:00
     */
    @ApiOperation(value = "post请求,返回自定义字符串，字符串长度可通过参数stringLength指定，默认1")
     ...
     * @date 2022/11/2 17:00
     */
    @ApiOperation(value = "post请求,返回异常，异常信息可通过参数exceptionDetails指定")
    @PostMapping("returnException")
    public String retrunException(@RequestParam(value = "exceptionDetails", defaultValue = "默认异常~") String exceptionDetails) throws Exception {
        logger.info("http的get请求返回异常");
        throw new Exception(exceptionDetails);
    }

    /**
     * @param responseTimeMs 返回响应时间控制
     * @author baili
     * @date 2022/11/2 17:00
     */
    @ApiOperation(value = "post请求,接口返回响应时间延迟，可通过参数responseTimeMs指定，默认延迟1ms")
    @PostMapping("returnResponseTimeController")
    public String returnResponseTimeController(@RequestParam(value = "responseTimeMs", defaultValue = "1") String responseTimeMs) throws Exception {
        logger.info("http的get请求返回响应时间控制");
