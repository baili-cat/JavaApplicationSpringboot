...
public class LinkStructure {
     ...
     */
    @ApiOperation(value = "get请求,通过参数调整单条链路中调用次数,hostIp需要指定调用的接口ip默认localhost,docker部署时可能会访问不通，callNumber调用次数")
    @GetMapping("MultipleLinkDynamicUrl")
    public String MultipleLinkDynamicUrl(@RequestParam(value = "hostIp", defaultValue = "localhost") String hostIp,
                               @RequestParam(value = "callNumber", defaultValue = "1") String callNumber,
                                         @RequestParam(value = "dynamicUrl", defaultValue = "/httpController/returnString") String dynamicUrl) {
        for (int i = 0; i < Integer.parseInt(callNumber); i++) {
            //http请求
            HttpRequestUtil.doGet("http://" + hostIp + dynamicUrl);
            logger.info("http://" + hostIp + dynamicUrl);
        }
        return "一条链路中http调用" + Integer.parseInt(callNumber) + "次";
    }
    ...
    public String MultipleLinkMiddleware(@RequestParam(value = "hostIp", defaultValue = "10.10.x.x:8080") String hostIp,
        ...
        String mysqlUrl = "http://" + hostIp + "/mysql/xSkyTest/queryTableInformation";
        ...
        return "一条链路中调用包含中间件" + middlewareNames + ",共调用" + callNumber + "次";
    }
}
