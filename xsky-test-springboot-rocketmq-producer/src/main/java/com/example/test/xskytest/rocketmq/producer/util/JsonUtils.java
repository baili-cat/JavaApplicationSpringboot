...
public class JsonUtils {
    ...
    public static <T> T jsonStr2Obj(String jsonStr, Class<T> clz) {
        try {
            ...
            log.error("jsonStr to obj error jsonString=" + jsonStr + " class " + clz.toString(), e);
            return null;
        } catch (JsonMappingException e) {
            log.error("jsonStr to obj error jsonString=" + jsonStr + " class " + clz.toString(), e);
            return null;
    ...
    public static <T> T jsonStr2Obj(String jsonStr, TypeReference<T> typeReference) {
        try {
            ...
            log.error("jsonString to obj error jsonString=" + jsonStr + " class " + typeReference.toString(), e);
            return null;
        } catch (JsonMappingException e) {
            log.error("jsonString to obj error jsonString=" + jsonStr + " class " + typeReference.toString(), e);
            return null;
    ...
    public static <T> T jsonObj2Obj(Object jsonObj, TypeReference<T> typeReference) {
        ...
        try {
            ...
            log.error("jsonObj to obj error jsonStr=" + jsonStr + " class " + typeReference.toString(), e);
            return null;
        } catch (JsonMappingException e) {
            log.error("jsonObj to obj error jsonStr=" + jsonStr + " class " + typeReference.toString(), e);
            return null;
