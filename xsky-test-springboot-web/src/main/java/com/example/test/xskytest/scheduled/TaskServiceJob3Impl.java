...
public class TaskServiceJob3Impl implements TaskService {
    ...
    public void HandlerJob() {
        ...
        try {
            ...
            log.info(new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(new Date()) + "  " + Thread.currentThread().getName() + "  任务三启动");
            log.info(httpTestControllerApplication.cxfNormal(cotrollerFuction.getHttpServletRequest()));
        } catch (Exception e) {
            log.error("job3 执行异常：{}", e.toString());
        }

        log.info(new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(new Date()) + "  " + Thread.currentThread().getName() + "  job3  结束");
    }
    ...
    public String methodName() {
        return "httpTestControllerApplication.cxfNormal";
    }
}
