...
@Service
@MyTaskService
public class TaskServiceJob6Impl implements TaskService {
    ...
    public void HandlerJob() {
        ...
        try {
            log.info("------job6 开始执行---------：" + new Date());
            log.info(new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(new Date()) + "  " + Thread.currentThread().getName() + "  任务六启动");
            log.info(httpTestControllerApplication.dubboAlibabaNormal(cotrollerFuction.getHttpServletRequest()));
        } catch (Exception e) {
            log.error("job6 执行异常：{}", e.toString());
        }

        log.info(new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(new Date()) + "  " + Thread.currentThread().getName() + "  job6  结束");
    }
    ...
    public String methodName() {
        return "httpTestControllerApplication.dubboAlibabaNormal";
    }
}
