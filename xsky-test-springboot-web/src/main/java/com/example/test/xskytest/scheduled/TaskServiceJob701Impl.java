...
@Service
@MyTaskService
public class TaskServiceJob701Impl implements TaskService {
    ...
    public void HandlerJob() {
        ...
        try {
            log.info("------job701 开始执行---------：" + new Date());
            log.info(new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(new Date()) + "  " + Thread.currentThread().getName() + "  任务七零一启动");
            log.info(httpTestExceptionControllerApplication.rocketmqException(cotrollerFuction.getHttpServletRequest()));
        } catch (Exception e) {
            log.error("job701 执行异常：{}", e.toString());
        }

        log.info(new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(new Date()) + "  " + Thread.currentThread().getName() + "  job701  结束");
    }
    ...
    public String methodName() {
        return "httpTestExceptionControllerApplication.rocketmqException";
    }
}
