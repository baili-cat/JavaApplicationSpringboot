...
@Service
@MyTaskService
public class TaskServiceJob702Impl implements TaskService {
    ...
    public void HandlerJob() {
        ...
        try {
            log.info("------job702 开始执行---------：" + new Date());
            log.info(new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(new Date()) + "  " + Thread.currentThread().getName() + "  任务七零二启动");
            log.info(httpTestExceptionControllerApplication.cxfException(cotrollerFuction.getHttpServletRequest()));
        } catch (Exception e) {
            log.error("job702 执行异常：{}", e.toString());
        }

        log.info(new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(new Date()) + "  " + Thread.currentThread().getName() + "  job702  结束");
    }
    ...
    public String methodName() {
        return "httpTestExceptionControllerApplication.cxfException";
    }
}
