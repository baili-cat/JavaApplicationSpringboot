...
@Service
@MyTaskService
public class TaskServiceJob707Impl implements TaskService {
    ...
    public void HandlerJob() {
        ...
        try {
            log.info("------job707 开始执行---------：" + new Date());
            log.info(new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(new Date()) + "  " + Thread.currentThread().getName() + "  任务七零七启动");
            log.info(httpTestExceptionControllerApplication.dubboApacheException(cotrollerFuction.getHttpServletRequest()));
        } catch (Exception e) {
            log.error("job707 执行异常：{}", e.toString());
        }

        log.info(new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(new Date()) + "  " + Thread.currentThread().getName() + "  job707  结束");
    }
    ...
    public String methodName() {
        return "httpTestExceptionControllerApplication.dubboApacheException";
    }
}
