...
@Service
@MyTaskService
public class TaskServiceJob709Impl implements TaskService {
    ...
    public void HandlerJob() {
        ...
        try {
            log.info("------job709 开始执行---------：" + new Date());
            log.info(new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(new Date()) + "  " + Thread.currentThread().getName() + "  任务七零九启动");
            log.info(httpTestExceptionControllerApplication.httpCustomerrortException(cotrollerFuction.getHttpServletRequest()));
        } catch (Exception e) {
            log.error("job709 执行异常：{}", e.toString());
        }

        log.info(new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(new Date()) + "  " + Thread.currentThread().getName() + "  job709  结束");
    }
    ...
    public String methodName() {
        return "httpTestExceptionControllerApplication.httpCustomerrortException";
    }
}
